...
//JW
//zio_checksum_generate -> zio to jw_zio
//remove barriers, no zio->chksm = 1;
//zio_checksum_compute -> new blk_cksum_tmp;
...
/*
static int
get_random_bytes_(char *buf, size_t bytes)
{
	int rand;
...
unsigned long long dd_t=0, dd_c=0;
static zio_t *
...
//JW_chk :0403
	...
	//ASSERT(taskq_empty_ent(&zio->chksm_io_tqent));
	ASSERT(taskq_empty_ent(&zio->io_tqent));
	//dprintf("[%u] chksm:%d \n", zio->id, zio->chksm);
/*
#ifdef _KERNEL
	if(zio->id < 700)
		printk(KERN_WARNING "ZIO_ori: name:%s nactive:%d [%u] \n", tq->tq_name, tq->tq_nactive, zio->id);
...
#ifdef _KERNEL
	if(zio->id < 700)
		printk(KERN_WARNING "ZIO_chk: name:%s nactive:%d [%u] \n", tq->tq_name, tq->tq_nactive, zio->id);
	//dprintf("[%u] chksm:%d tq_name:%s tq_nthreads:%d tq_nactive:%d \n", zio->id, zio->chksm, tq->tq_name, tq->tq_nthreads, tq->tq_nactive);
#endif
*/	
	//chksm_taskq_dispatch_ent(tq, (task_func_t *)chksm_zio_checksum_generate, zio, flags, &zio->chksm_io_tqent);
	
	
	
	//JW0701
	
	zio_t *chksm_zio = kmem_cache_alloc(zio_cache, KM_SLEEP);
	bzero(chksm_zio, sizeof(zio_t));
	taskq_init_ent(&chksm_zio->io_tqent);
	chksm_zio->id = 0;

	//chksm_zio->io_bp = (blkptr_t *)zio->io_bp;a
	if(zio->io_bp != NULL)
		memcpy((void *)chksm_zio->io_bp, (void *)zio->io_bp, sizeof(zio->io_bp));
	chksm_zio->io_prop = zio->io_prop;
	chksm_zio->io_child_type = zio->io_child_type;

	//chksm_zio->io_abd = (struct abd *)zio->io_abd;
	if(zio->io_abd != NULL)
		memcpy((void *)chksm_zio->io_abd, (void *)zio->io_abd, sizeof(zio->io_abd));
	chksm_zio->io_size = zio->io_size;

	chksm_zio->io_offset = zio->io_offset;
	//chksm_zio->io_spa = (spa_t *)zio->io_spa;
	if(zio->io_spa != NULL)
		memcpy((void *)chksm_zio->io_spa, (void *)zio->io_spa, sizeof(zio->io_spa));


	chksm_taskq_dispatch_ent(tq, (task_func_t *)chksm_zio_checksum_generate, chksm_zio, flags, &chksm_zio->io_tqent, chksm_zio->id);



	//chksm_zio_checksum_generate(zio);
	//zio->chksm = 2;
	
	//spa_taskq_dispatch_ent(spa, t, q, (task_func_t *)zio_checksum_generate, zio,
	//    flags, &zio->chksm_io_tqent);
}
...
unsigned long long cc_t=0, cc_c=0;
static zio_t *
zio_issue_async(zio_t *zio)
{
hrtime_t cc_local[2];
cc_local[0] = gethrtime();
	zio_taskq_dispatch(zio, ZIO_TASKQ_ISSUE, B_FALSE);
cc_local[1] = gethrtime();
calclock(cc_local, &cc_t, &cc_c);
	return (NULL);
}
...
//JW
		...
		t = zio->io_type;
		q = ZIO_TASKQ_ISSUE;
		tqs = &spa->spa_zio_taskq[t][q];

		//if (zio_taskq_member(zio, q)) {
		tq = tqs->stqs_taskq[0];
#ifdef _KERNEL
			/*active = tq->tq_nactive;
			ctxt_tmp = get_ctxt_zfs(tq);
			if(ctxt_tmp != tq->tq_ctxt){
				ctxt = ctxt_tmp - tq->tq_ctxt;
				tq->tq_ctxt = ctxt_tmp;
				//dprintf("[jjww][%d][%d] stage:%d name:%s active:%d\n", ctxt, tq->tq_ctxt, highbit64(stage)-1, tq->tq_name, active);	
			}*/

		if(x) {
			dprintf("[xx] zio_opt:%d zio_nactive:%d zio_ctxt:%d\n", zio_opt, zio_nactive, zio_ctxt);
			x=0;
		}

		if(zio_opt == 1 && highbit64(stage)-1 == 4 && tq->tq_nactive >= zio_nactive){
			ctxt_tmp = get_ctxt_zfs(tq);
			if(ctxt_tmp != tq_ctxt){
			...
			if (tq->tq_ctxt >= zio_ctxt){
				a++;
				stage <<= 1;
				//dprintf("[jw][%d][%d] ctxt:%d name:%s nthreads:%d active:%d\n", a, b, tq->tq_ctxt, tq->tq_name, tq->tq_nthreads, tq->tq_nactive);
			}
			else{
				b++;
				//dprintf("[wj][%d][%d] ctxt:%d name:%s nthreads:%d active:%d\n", a, b, ctxt, tq->tq_name, tq->tq_nthreads,  active);
			}
			dprintf("[jw][%d][%d] ctxt:%d name:%s active:%d lsize:%d size:%d orig_size:%d\n", 
				a, b, tq->tq_ctxt, tq->tq_name, tq->tq_nthreads, tq->tq_nactive, 
				zio->io_lsize, zio->io_size, zio->io_orig_size);
		}
//JW_chk
//
//
...
c_local[0] = gethrtime();
	...
	mutex_enter(&zio->io_lock);
#ifdef _KERNEL
	//printk(KERN_WARNING "WAIT: [%u] chksm:%d\n", zio->id, zio->chksm);
#endif
	
	while (zio->io_executor != NULL)
		cv_wait_io(&zio->io_cv, &zio->io_lock);
#ifdef _KERNEL
	//printk(KERN_WARNING "WAIT-OUT: [%u] chksm:%d\n", zio->id, zio->chksm);
#endif
...
{
	//dprintf("SUSPEND - [%u]zio->chksm:%d zio->io_error:%d\n", zio->id, zio->chksm, zio->io_error);
...
unsigned long long ff_t=0, ff_c=0;
static zio_t *
...
#ifdef _KERNEL
	//printk(KERN_WARNING "JW-ZFS-DVA: [%u] chksm:%d\n", zio->id, zio->chksm);
#endif
hrtime_t ff_local[2];
ff_local[0] = gethrtime();
	
	if (zio->io_priority == ZIO_PRIORITY_SYNC_WRITE ||
	    !spa_normal_class(zio->io_spa)->mc_alloc_throttle_enabled ||
	    zio->io_child_type == ZIO_CHILD_GANG ||
	    zio->io_flags & ZIO_FLAG_NODATA) {
ff_local[1] = gethrtime();
calclock(ff_local, &ff_t, &ff_c);
		return (zio);
	}
...
unsigned long long chk_t=0, chk_c=0;
void
...
#endif
chk_local[1] = gethrtime();
calclock(chk_local, &chk_t, &chk_c);
	//if(zio->chksm != 1)
	zio_taskq_dispatch(zio, ZIO_TASKQ_ISSUE, B_TRUE);
}

unsigned long long gg_t=0, gg_c=0;
static zio_t *
...
gg_local[1] = gethrtime();
calclock(gg_local, &gg_t, &gg_c);
...
unsigned long long hh_t=0, hh_c=0;
static zio_t *
zio_dva_free(zio_t *zio)
{
hrtime_t hh_local[2];
hh_local[0] = gethrtime();
	metaslab_free(zio->io_spa, zio->io_bp, zio->io_txg, B_FALSE);
hh_local[1] = gethrtime();
calclock(hh_local, &hh_t, &hh_c);
	return (zio);
}
 ...
 */
unsigned long long jj_t=0, jj_c=0;
unsigned long long xx_t=0, xx_c=0;
//unsigned long long chk_t_1=0, chk_c_1=0;
static zio_t *
zio_vdev_io_start(zio_t *zio)
{
hrtime_t jj_local[2];
jj_local[0] = gethrtime();
	vdev_t *vd = zio->io_vd;
	uint64_t align;
	spa_t *spa = zio->io_spa;
...
unsigned long long kk_t=0, kk_c=0;
static zio_t *
zio_vdev_io_done(zio_t *zio)
{
hrtime_t kk_local[2];
kk_local[0] = gethrtime();
	vdev_t *vd = zio->io_vd;
	vdev_ops_t *ops = vd ? vd->vdev_ops : &vdev_mirror_ops;
	boolean_t unexpected_error = B_FALSE;

	if (zio_wait_for_children(zio, ZIO_CHILD_VDEV_BIT, ZIO_WAIT_DONE)) {
kk_local[1] = gethrtime();
calclock(kk_local, &kk_t, &kk_c);
		return (NULL);
	}
...
unsigned long long ll_t=0, ll_c=0;
static zio_t *
zio_vdev_io_assess(zio_t *zio)
{
hrtime_t ll_local[2];
ll_local[0] = gethrtime();
	vdev_t *vd = zio->io_vd;

	if (zio_wait_for_children(zio, ZIO_CHILD_VDEV_BIT, ZIO_WAIT_DONE)) {
ll_local[1] = gethrtime();
calclock(ll_local, &ll_t, &ll_c);
		return (NULL);
	}
 ...
 * ==========================================================================
 */
unsigned long long ee_t=0, ee_c=0;
static zio_t *
...
hrtime_t ee_local[2];
ee_local[0] = gethrtime();
	//dprintf("[%u] original_chksm:%d\n", zio->id, zio->chksm);
	if (bp == NULL) {
...
ee_local[1] = gethrtime();
calclock(ee_local, &ee_t, &ee_c);
			return (zio);
		}
...
calclock(ee_local1, &ee_t1, &ee_c1);
		...
		if (BP_IS_GANG(bp) && zio->io_child_type == ZIO_CHILD_GANG) {
			ASSERT(!IO_IS_ALLOCATING(jw_zio));
			checksum = ZIO_CHECKSUM_GANG_HEADER;
		} else {
			checksum = BP_GET_CHECKSUM(bp);
		}
...
//0616
	...
	*/
	//if(zio->chksm == 2)
	//dprintf("[%u]zio->chksm:%d zio->io_error:%d\n", zio->id, zio->chksm, zio->io_error);
	return (zio);
}
 ...
 * ==========================================================================
 */
unsigned long long ii_t=0, ii_c=0;
unsigned long long iia_t=0, iia_c=0;
unsigned long long iib_t=0, iib_c=0;
unsigned long long iic_t=0, iic_c=0;
unsigned long long iid_t=0, iid_c=0;

unsigned long long enter_t=0, enter_c=0;
unsigned long long exit_t=0, exit_c=0;

unsigned long long iiba_t=0, iiba_c=0;
unsigned long long iibb_t=0, iibb_c=0;
unsigned long long iibc_t=0, iibc_c=0;
...
ii_local[0] = gethrtime();
	...
	if (zio_wait_for_children(zio, ZIO_CHILD_GANG_BIT | ZIO_CHILD_DDT_BIT,
	    ZIO_WAIT_READY)) {
ii_local[1] = gethrtime();
calclock(ii_local, &ii_t, &ii_c);
...
hrtime_t iiba_local[2];
iiba_local[0] = gethrtime();	
//JW: calling arc_write_ready
		zio->io_ready(zio);
...
iib_local[1] = gethrtime();
calclock(iib_local, &iib_t, &iib_c);
...
unsigned long long m_t=0, m_c=0;
static zio_t *
zio_done(zio_t *zio)
{
	/*
...
MODULE_PARM_DESC(zio_opt, "zio optimization on/off");
...
MODULE_PARM_DESC(zio_ctx,
	"Number of context switches threshold of dispatching extra threads");
